...
def get_optimal_A(B, A, present, cl, L, idxs=[], reg=0):
    ...
    ----------
        ...
        present: boolean attention_mask of present data
        cl: matrix cl
    ...
    for i in idxs:
        ...
        try:
            A[i, :] = (
                Bi @ np.linalg.lstsq(Bi.T @ Bi / L + lmbda, Xi / L, rcond=0)[0]
            )
...
def fit_factors_and_loadings(
    ...
    char_panel[np.sum(~np.isnan(missing_mask_overall), axis=2) < min_chars] = (
        np.nan
    )
    ...
    assert (
        np.sum(np.isnan(lmbda)) == 0
    ), f"lambda should contain no nans, {np.argwhere(np.isnan(lmbda))}"
    ...
    if run_in_parallel:
        gammas = [
            x
            for x in Parallel(n_jobs=30, verbose=5)(
                delayed(get_gamma_t)(
                    ct=char_panel[t],
    ...
    if time_varying_lambdas:
        new_imputation = np.concatenate(
            [np.expand_dims(x @ l.T, axis=0) for x, l in zip(gamma_ts, lmbda)],
            axis=0,
        )
    else:
        new_imputation = np.concatenate(
            [np.expand_dims(x @ lmbda.T, axis=0) for x in gamma_ts],
            axis=0,
        )
    ...
    if eval_data is not None:
        print(
            f"eval resids rmse are ",
            np.sqrt(np.nanmean(np.square(new_imputation - eval_data))),
        )
...
def estimate_lambda(
    ...
    min_char_mask = np.expand_dims(
        np.sum(~np.isnan(char_panel), axis=2) >= min_chars,
        axis=2,
    )
    ...
    if time_varying_lambdas:
        ...
        for t in range(len(cov_mats)):
            ...
            lmbda.append(
                eig_vects[:, idx[:K]]
                * np.sqrt(np.maximum(eig_vals[idx[:K]].reshape(1, -1), 0)),
            )
        ...
        idx = np.abs(eig_vals).argsort()[::-1]
        lmbda = eig_vects[:, idx[:K]] * np.sqrt(
            eig_vals[idx[:K]].reshape(1, -1),
        )
...
def get_sufficient_statistics_last_val(
...
def impute_chars(
    ...
    if suff_stat_method == "last_val":
        suff_stats, _ = get_sufficient_statistics_last_val(
            char_data,
            max_delta=None,
            residuals=residuals,
        )
...
def impute_beta_combined_regression(
    characteristics_panel,
    xs_imps,
    ...
    for l in tqdm(range(L)):
        ...
        for t in range(T):
            ...
            if xs_imps is not None:
                suff_stat = np.concatenate(
                    [xs_imps[t, :, l : l + 1], sufficient_statistics[t, :, l]],
                    axis=1,
                )
            else:
            ...
            available_for_fit = np.logical_and(
                ~np.isnan(characteristics_panel[t, :, l]),
        ...
        if constant_beta:
            ...
            for t in range(T):
                beta_l_t = np.linalg.lstsq(
                    fit_suff_stats[inds[t] : inds[t + 1]],
                    fit_tgts[inds[t] : inds[t + 1]],
                    rcond=None,
                )[0]
                betas[t, l, :] = beta_l_t
        ...
        for t in range(T):
            ...
            available_for_imputation = np.all(~np.isnan(suff_stat), axis=1)
            imputed_data[t, available_for_imputation, l] = (
                suff_stat[available_for_imputation, :] @ beta_l_t
            )
...
def get_oos_estimates_given_loadings(
    chars,
    reg,
    Lmbda,
    time_varying_lmbda=False,
    get_factors=False,
):
    """
    ...
    def impute_t(t_chars, reg, C, Lmbda, get_factors=False):
        if not get_factors:
            ...
            imputation = (
                np.zeros((t_chars.shape[0], t_chars.shape[1], Lmbda.shape[1]))
                * np.nan
            )
        ...
        for n in range(t_chars.shape[0]):
            if net_mask[n] == 1:
                ...
                for i in range(C):
                    ...
                    X = Lmbda[mask[n], :]
                    L = np.eye(K) * reg
                    params = np.linalg.lstsq(X.T @ X + L, X.T @ y, rcond=None)[
                        0
                    ]
                    if get_factors:
    ...
    if time_varying_lmbda:
        imputation = list(
            Parallel(n_jobs=60)(
                delayed(impute_t)(chars_t, reg, C, l, get_factors=get_factors)
...
def run_imputation(
    ...
    if time_varying_loadings:
        xs_imputations = np.concatenate(
            [np.expand_dims(g @ l.T, axis=0) for g, l in zip(gamma_ts, lmbda)],
            axis=0,
        )
    else:
        xs_imputations = np.concatenate(
            [np.expand_dims(g @ lmbda.T, axis=0) for g in gamma_ts],
            axis=0,
        )
...
def impute_data_xs(
    ...
    return run_imputation(
        characteristics,
        n_xs_factors=n_xs_factors,
        time_varying_loadings=time_varying_loadings,
        xs_factor_reg=xs_factor_reg,
        use_bw_ts_info=False,
        include_ts_residuals=False,
        min_xs_obs=min_xs_obs,
    )
